...
Page({
  ...
  recorderStop: function(event) {
    ...
    app.uploadVoice(recorder,(recordid) => {
      ...
      question_detail[index].recorderurl = recorder;
      that.setData({
        data
      })
    });
  ...
  onSub() {
    ...
    _data.detail = {
      ...
      total_score:this.data.satisanswer,
      suggest:this.data.othertext
    }
    _data.detail.result = JSON.stringify(_data.detail.result)
    console.log('提交数据', _data.detail);
    if (this.isUpload) {
      wx.showToast({
        icon: 'none',
    ...
    util.PostRequest('Evaluate/submission',_data.detail,res=>{
