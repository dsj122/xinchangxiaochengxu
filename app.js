...
App({
  ...
  uploadVoice(path,cb) {
    let that = this;
    let url = util.host+'Evaluate/uploadVoice';
    console.log('path222', path);
    wx.uploadFile({
      url: url,
      filePath: path,
  ...
  //获取地址参数
  getURLdata(url,value){
    console.log(url,'url');
    
    let dataList= url.split('?')[1].split('&') //获取参数数据
